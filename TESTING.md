...
- Zustand store in tests
  ...
  - Example:
    ...
    it("opens a new request tab", async () => {
      ...
      const collection = collectionsApi.addCollection("My Col")
      const request = collectionsApi.createRequest(collection.id)
      await requestTabsApi.loadTab(collection.id, request.id)
...
- End-to-end tests: `yarn test:e2e` runs WebDriver-driven UI flows against the packaged app for user-focused coverage (excludes auto-doc capture).
- Auto-doc capture flow: `yarn test:e2e:auto-docs` runs the screenshot/documentation generator spec independently.
