...
### Collections API Surface (2025-10-19)

- React views must call `useCollection(collectionId)` (or `useCollections`) to read data; it synchronously returns the cached collection and will trigger a load via `collectionsApi().loadCollection(id)` on first access. Only call `collectionsApi().getCollection(id)` from non-React code once the collection has been loaded.
- The synchronous `collectionsApi` surface is grouped as follows:
  - **Index + persistence:** `getCollectionsIndex`, `reorderCollections`, `loadCollection`, `saveCollection`, `getCollection`, `clearScratchCollection`.
  - **Collection lifecycle:** `addCollection`, `updateCollection`, `removeCollection`, `importCollection`, `mergeCollection`, `exportCollection`.
  - **Request operations:** `getRequest`, `createRequest`, `duplicateRequest`, `deleteRequest`, `moveRequestToFolder`, `reorderRequestsInFolder`, `updateRequest`, `commitRequestPatch`, `discardRequestPatch`, plus granular mutators (`updateRequestPatchQueryParam`, `updateRequestPatchPathParam`, `updateRequestPatchHeader`, `updateRequestPatchCookieParam`, `setRequestBodyFormField`, `updateRequestBody`, `setRequestAuthentication`, `updateRequestOptions`, `setRequestAutoSave`, `setRequestName`, `setRequestMethod`, `setRequestUrl`).
  - **Folder operations:** `createFolder`, `renameFolder`, `deleteFolder`, `moveFolder`, `reorderFolders`.
  - **Environment management (via `EnvironmentsApi`):** `createEnvironment`, `updateEnvironment`, `deleteEnvironment`, `setActiveEnvironment`, `addEnvironmentVariable`, `updateEnvironmentVariable`, `deleteEnvironmentVariable`.
- All mutators run synchronously against the in-memory cache; persistence is handled internally and remains transparent to callers.
