...
fn parse_token_response_body(body: &[u8]) -> Result<TokenResponseWire, AppError> {
    ...
    log::debug!(
        ...
        body.len(),
        if body.len() > 200 {
            format!("{}... (truncated)", &as_str.chars().take(200).collect::<String>())
        } else {
            as_str.to_string()
        }
    );
    ...
    if let Ok(value) = serde_json::from_slice::<serde_json::Value>(body) {
        log::debug!("[AUTH] Token response parsed as JSON");
        ...
        log::debug!(
            "[AUTH] JSON parsed but missing required fields (access_token or token_type). Trying form-encoded format."
        );
        // If JSON parsed but required fields missing, fall through to urlencoded parser
    } else {
        log::debug!("[AUTH] Failed to parse token response as JSON, attempting form-urlencoded");
    }
    ...
    {
        log::debug!(
            "[AUTH] Token response parsed as form-urlencoded with {} fields",
            form_map.len()
        );
        let at = form_map
            .get("access_token")
        ...
        log::debug!(
            "[AUTH] Form-encoded response parsed but missing required fields. Found fields: {:?}",
...
pub async fn discover_oidc(app: AppHandle, url: String) -> Result<OidcDiscovery, AppError> {
    ...
    log::debug!(
        ...
        wire.device_authorization_endpoint.as_deref().unwrap_or("none")
    );
        ...
        AuthConfig::Oauth2 {
            ...
            match grant_type.as_str() {
                "client_credentials" => {
                    ...
                    match chosen_auth {
                        ClientAuth::Basic => {
                            log::debug!("[AUTH] Using HTTP Basic authentication for client credentials");
                        ...
                        ClientAuth::Body => {
                            log::debug!("[AUTH] Placing client credentials in request body");
                    ...
                    if let Some(extra) = &token_extra_params {
                        ...
                        log::debug!("[AUTH] No extra OAuth2 parameters");
                    }
                ...
                "refresh_token" => {
                    ...
                    match chosen_auth {
                        ClientAuth::Basic => {
                            log::debug!("[AUTH] Using HTTP Basic authentication for refresh_token");
                            if !client_id.is_empty() && !client_secret.is_empty() {
                                let raw = format!("{client_id}:{client_secret}");
                                let b64 = general_purpose::STANDARD.encode(raw);
                                headers.insert("Authorization".to_string(), format!("Basic {b64}"));
                        ...
                        ClientAuth::Body => {
                            log::debug!("[AUTH] Placing client credentials in request body for refresh_token");
                            params.push(("client_id", &client_id));
                            params.push(("client_secret", &client_secret));
                        }
                    }
                    ...
                    if let Some(extra) = &token_extra_params {
                        log::debug!("[AUTH] Adding {} extra parameters to refresh_token request", extra.len());
                        for (k, v) in extra {
                            params.push((k.as_str(), v.as_str()));
                        }
                    }
...
async fn handle_device_code(
    ...
    if matches!(client_auth, ClientAuth::Basic) {
        log::debug!("[AUTH] device_code: Using HTTP Basic authentication");
        if let Some(secret) = client_secret.as_ref() {
            let raw = format!("{client_id}:{secret}");
            let b64 = general_purpose::STANDARD.encode(raw);
            headers.insert("Authorization".to_string(), format!("Basic {b64}"));
    ...
    log::debug!(
        "[AUTH] device_code: Device code response parsed. User code: {}, Expires: {:?}s, Poll interval: {:?}s",
        device_payload.user_code,
        device_payload.expires_in,
        device_payload.interval
    );
    ...
    loop {
        if chrono::Utc::now().timestamp() > expires_at {
            log::error!("[AUTH] device_code: Device authorization expired after {} polling attempts", poll_attempt);
            return Err(AppError::new(
                ErrorKind::Timeout,
        ...
        log::debug!("[AUTH] device_code: Sleeping for {}s before poll attempt {}", interval, poll_attempt + 1);
        sleep(Duration::from_secs(interval)).await;
        poll_attempt += 1;
        log::debug!("[AUTH] device_code: Starting poll attempt {} (interval: {}s)", poll_attempt, interval);
        ...
        let response = engine
            ...
            .await
            .map_err(|e| {
                log::error!("[AUTH] device_code: Poll attempt {} HTTP request failed: {}", poll_attempt, e);
                AppError::new(ErrorKind::HttpError, e.to_string())
            })?;
