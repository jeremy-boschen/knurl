...
pub fn get_or_create_key(app: &AppHandle, key_name: &str) -> Result<[u8; 32], AppError> {
    if should_disable_keyring() {
        warn!("Keyring access disabled; using file-based key storage");
        return load_or_create_fallback_key(app, key_name);
    }

    let keyring_attempt =
        panic::catch_unwind(AssertUnwindSafe(|| -> Result<[u8; 32], AppError> {
            ...
            if let Ok(encoded) = entry.get_password() {
                let decoded = b64::URL_SAFE_NO_PAD
                    ...
                    .map_err(|e: DecodeError| {
                        app_error!(ErrorKind::KeyringBadEncoding, e.to_string())
                    })?;
    ...
    match keyring_attempt {
        Ok(Ok(key)) => Ok(key),
        Ok(Err(err)) => {
            if matches!(
                err.kind,
                ErrorKind::KeyringPlatformFailure | ErrorKind::KeyringAttributeInvalid
            ) {
                warn!(
                    "Keyring unavailable ({}); falling back to file-based key storage",
                    err.message
                );
                load_or_create_fallback_key(app, key_name)
            } else {
                Err(err)
            }
        }
        Err(_) => {
            warn!("Keyring backend panicked; falling back to file-based key storage");
            load_or_create_fallback_key(app, key_name)
        }
    }
}
...
#[cfg(not(test))]
fn should_disable_keyring() -> bool {
    let disable_env = std::env::var("KNURL_DISABLE_KEYRING")
        .ok()
        .and_then(|value| parse_bool(&value));
    if disable_env == Some(true) {
        return true;
    }

    if std::env::var("KNURL_USE_KEYRING")
        .ok()
        .and_then(|value| parse_bool(&value))
        == Some(false)
    {
    ...
    if std::env::var("WSL_DISTRO_NAME").is_ok()
        && disable_env.is_none()
        && std::env::var("KNURL_USE_KEYRING").is_err()
    {
        return true;
    }
...
#[cfg(not(test))]
fn load_or_create_fallback_key(app: &AppHandle, key_name: &str) -> Result<[u8; 32], AppError> {
    let keys_dir = loader::resolve_app_data_dir(app)?.join(".keys");
    fs::create_dir_all(&keys_dir)?;
    let key_path = keys_dir.join(format!("{key_name}.key"));

    if key_path.exists() {
        let encoded = fs::read_to_string(&key_path)?;
        if let Ok(decoded) = b64::URL_SAFE_NO_PAD.decode(encoded.trim()) {
            let len = decoded.len();
            if let Ok(key) = <[u8; 32]>::try_from(decoded) {
                return Ok(key);
            }
            warn!(
                "Fallback key file at '{}' contained {} bytes; regenerating",
                key_path.display(),
                len
            );
        } else {
            warn!(
                "Failed to decode fallback key file at '{}'; regenerating",
                key_path.display()
            );
        }
    }
