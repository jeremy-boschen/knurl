...
function validateClientSecret(client, auth) {
  if (client.clientSecret) {
    if (auth.method === "none") {
      return {
        ok: false,
        error: { status: 401, body: { error: "invalid_client", error_description: "client authentication required" } },
      }
    }
    if (auth.clientSecret !== client.clientSecret) {
      return {
        ok: false,
        error: { status: 401, body: { error: "invalid_client", error_description: "client secret mismatch" } },
      }
    }
    return { ok: true }
  }
  if (auth.method === "basic" && auth.clientSecret) {
    return {
      ...
      error: {
        status: 401,
        body: { error: "invalid_client", error_description: "public client must not provide secret" },
      },
    }
  }
  return { ok: true }
...
async function main() {
  ...
  wrapNamedHandler(
    ...
    (original) => async (req, res, next) => {
      console.debug("[DISCOVERY ENDPOINT] OpenID Connect discovery requested")
      const originalJson = res.json.bind(res)
      res.json = (body) => {
        console.debug("[DISCOVERY ENDPOINT] Sending configuration:", {
          issuer: body.issuer,
  ...
  wrapNamedHandler(app, "/authorize", "authorizeHandler", (original) => async (req, res, next) => {
    console.debug("[AUTHORIZE ENDPOINT] Query params:", {
...
<body>
  ...
  app.get("/mock/image/png", (_req, res) => {
    // 1x1 transparent PNG
  ...
  app.get("/mock/github/user/repos", (_req, res) => {
    res.setHeader("Content-Type", "application/json")
    respondJson(res, 200, [
      {
        id: 1,
  ...
  app.get("/mock/github/repos/:owner/:repo/issues", (req, res) => {
    const { owner: _owner, repo: _repo } = req.params
    const state = req.query.state || "open"
    const _labels = req.query.labels ? String(req.query.labels).split(",") : []
  ...
  app.get("/mock/github/user", (_req, res) => {
    res.setHeader("Content-Type", "application/json")
    respondJson(res, 200, {
  ...
  app.get("/mock/github/repos/:owner/:repo/issues/:number/comments", (req, res) => {
    const { owner, repo, number: _number } = req.params
    res.setHeader("Content-Type", "application/json")
    respondJson(res, 200, [
      {
        id: 1,
  ...
  app.get("/mock/github/repos/:owner/:repo/commits/:sha", (req, res) => {
    ...
    respondJson(res, 200, {
      ...
      tree: {
        sha: "treeSha123",
        url: `http://localhost:3000/mock/github/repos/${owner}/${repo}/git/trees/treeSha123`,
      },
  ...
  app.get("/mock/github/search/repositories", (_req, res) => {
    res.setHeader("Content-Type", "application/json")
    respondJson(res, 200, {
      total_count: 1000,
  ...
  app.get("/mock/github/search/issues", (_req, res) => {
    res.setHeader("Content-Type", "application/json")
    respondJson(res, 200, {
      total_count: 42,
      incomplete_results: false,
  ...
  app.get("/mock/github/user/gists", (_req, res) => {
    res.setHeader("Content-Type", "application/json")
    respondJson(res, 200, [
      {
        id: "abc123",
