...
export const OAuth2Editor: FC<OAuth2EditorProps> = ({ auth, onUpdate, onDiscover, token }) => {
  ...
  return (
    <div className="space-y-4" data-test-id="oauth2-editor">
      <Field label="Discovery URL">
        {(id) => (
          <Input
            ...
            data-test-id="oauth2-editor:discovery-url-input"
            endAddon={
              <Button variant="ghost" size="sm" onClick={onDiscover} data-test-id="oauth2-editor:discover-button">
                Discover
              </Button>
            }
          />
      ...
      <Field label="Grant Type">
        {(id) => (
          <Select value={grantType} onValueChange={(value) => onUpdate({ grantType: value })}>
            <SelectTrigger id={id} className="w-full text-sm" data-test-id="oauth2-editor:grant-type-select">
              <SelectValue placeholder="Select grant type" />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="client_credentials" data-test-id="oauth2-editor:grant-type-option:client_credentials">
                Client Credentials
              </SelectItem>
              <SelectItem value="authorization_code" data-test-id="oauth2-editor:grant-type-option:authorization_code">
                Authorization Code (PKCE)
              </SelectItem>
              <SelectItem value="refresh_token" data-test-id="oauth2-editor:grant-type-option:refresh_token">
                Refresh Token
              </SelectItem>
              <SelectItem value="device_code" data-test-id="oauth2-editor:grant-type-option:device_code">
                Device Code
              </SelectItem>
            </SelectContent>
          </Select>
      ...
      <Field label="Token URL">
        {(id) => (
          <Input
            ...
            className="w-full font-mono"
            data-test-id="oauth2-editor:token-url-input"
          />
        )}
      </Field>
          ...
          <Field label="Client ID">
            {(id) => (
              <Input
                ...
                className="w-full font-mono"
                data-test-id="oauth2-editor:client-id-input"
              />
            )}
          </Field>
          ...
          <Field label="Redirect URI">
            {(id) => (
              <Input
                ...
                onChange={(e) => onUpdate({ redirectUri: e.target.value })}
                className="w-full font-mono"
                data-test-id="oauth2-editor:redirect-uri-input"
              />
            )}
          </Field>
          <Field label="PKCE">
            {(id) => (
              <Select
                value={(auth.usePkce ?? true) ? "on" : "off"}
                onValueChange={(value) => onUpdate({ usePkce: value === "on" })}
              >
                <SelectTrigger id={id} className="w-full text-sm" data-test-id="oauth2-editor:pkce-select">
                  <SelectValue placeholder="Select PKCE behaviour" />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="on" data-test-id="oauth2-editor:pkce-option:on">
                    Enabled (S256)
                  </SelectItem>
                  <SelectItem value="off" data-test-id="oauth2-editor:pkce-option:off">
                    Disabled
                  </SelectItem>
                </SelectContent>
              </Select>
      ...
      {grantType === "device_code" && (
        <Field label="Device Authorization URL">
          {(id) => (
            <Input
              id={id}
              type="text"
              value={auth.deviceAuthorizationUrl ?? ""}
              onChange={(e) => onUpdate({ deviceAuthorizationUrl: e.target.value })}
              className="w-full font-mono"
              data-test-id="oauth2-editor:device-url-input"
            />
          )}
        </Field>
      )}
        ...
        <Field label="Token Strategy">
          {(id) => (
            <Select value={auth.tokenCaching ?? "always"} onValueChange={(value) => onUpdate({ tokenCaching: value })}>
              <SelectTrigger id={id} className="w-full text-sm" data-test-id="oauth2-editor:token-strategy-select">
                <SelectValue placeholder="Select token strategy" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="always" data-test-id="oauth2-editor:token-strategy-option:always">
                  Use cached if available
                </SelectItem>
                <SelectItem value="never" data-test-id="oauth2-editor:token-strategy-option:never">
                  Always refresh (write cache)
                </SelectItem>
              </SelectContent>
            </Select>
          )}
            ...
            <div className="space-y-1">
              <div className="flex items-center gap-2">
                <Input
                  ...
                  value={token.value}
                  className="w-full font-mono"
                  data-test-id="oauth2-editor:access-token-input"
                />
                {token.onFetch && (
                  <Button
                    variant="secondary"
                    size="sm"
                    onClick={token.onFetch}
                    data-test-id="oauth2-editor:fetch-token-button"
                  >
                    Fetch
                  </Button>
                )}
                {token.onDelete && (
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={token.onDelete}
                    disabled={!token.value}
                    data-test-id="oauth2-editor:delete-token-button"
                  >
                    Delete
                  </Button>
                )}
        ...
        <Field label="Refresh Token">
          {(id) => (
            <Input
              ...
              className="w-full font-mono"
              data-test-id="oauth2-editor:refresh-token-input"
            />
          )}
        </Field>
      )}
