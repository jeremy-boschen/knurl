...
describe("RequestAuthPanel", () => {
  ...
  it("updates the request patch when basic auth fields are changed", () => {
    ...
    expect(mockSetRequestAuthentication).toHaveBeenCalledWith("col-1", "req-1", {
      type: "basic",
  ...
  it("bearer custom scheme shows input and updates", async () => {
    ...
    expect(mockSetRequestAuthentication).toHaveBeenCalledWith(
      "col-1",
      "req-1",
      expect.objectContaining({ type: "bearer", bearer: expect.objectContaining({ scheme: "Token" }) }),
    )
  })
