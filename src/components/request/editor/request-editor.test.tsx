...
const updateRequestOptionsMock = vi.fn()
const setRequestAutoSaveMock = vi.fn()
const setRequestAuthenticationMock = vi.fn()
const setRequestNameMock = vi.fn()
const setRequestMethodMock = vi.fn()
const setRequestUrlMock = vi.fn()

const addPathParamMock = vi.fn()
const addQueryParamMock = vi.fn()
const addCookieParamMock = vi.fn()
const addHeaderMock = vi.fn()
const updateBodyMock = vi.fn()
const bodyAddFormItemMock = vi.fn()
const updateFormItemMock = vi.fn()
...
describe("RequestEditor", () => {
    ...
    useCollections.mockReturnValue({
      ...
      actions: {
        collectionsApi: () => ({
          ...
          setRequestAuthentication: setRequestAuthenticationMock,
          updateRequestOptions: updateRequestOptionsMock,
          setRequestAutoSave: setRequestAutoSaveMock,
          setRequestName: setRequestNameMock,
  ...
  it("updates body type and adds form fields through the Body tab dropdown", async () => {
    ...
    expect(updateBodyMock).toHaveBeenCalledWith({ type: "text", language: "json", content: "" })
    ...
    expect(updateBodyMock).toHaveBeenCalledWith({ encoding: "multipart" })
    expect(updateFormItemMock).toHaveBeenCalledWith(
      "mock-field",
      expect.objectContaining({ id: "mock-field", kind: "file" }),
    )
    expect(updateBodyMock).toHaveBeenCalledTimes(2)
  })
  ...
  it("updates authentication via the Auth tab dropdown", async () => {
    ...
    expect(setRequestAuthenticationMock).toHaveBeenCalledWith("col-1", "req-1", {
      type: "basic",
      basic: {},
    })
  })
