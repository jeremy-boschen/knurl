  ...
  const doImport = async () => {
    ...
    try {
      ...
      for (const collectionMeta of collectionsIndex) {
        await collectionsApi().loadCollection(collectionMeta.id)
      }
    ...
    <>
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent>
          ...
          <Textarea
            ...
            rows={3}
            className="font-mono"
            data-test-id="settings-data:import-key-textarea"
          />
          ...
          <DialogFooter>
            <Button
              ...
              disabled={isLoading}
              data-test-id="settings-data:import-cancel-button"
            >
              Cancel
            </Button>
            <Button
              onClick={startImport}
              disabled={isLoading || !newKeyInput.trim()}
              data-test-id="settings-data:import-confirm-button"
            >
              Confirm Import
            </Button>
          </DialogFooter>
      ...
      <AlertDialog open={confirmOpen} onOpenChange={(v) => setState((s) => ({ ...s, confirmOpen: v }))}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel disabled={isLoading} data-test-id="settings-data:confirm-import-cancel">
              Cancel
            </AlertDialogCancel>
            <AlertDialogAction
              onClick={doImport}
              disabled={isLoading}
              data-test-id="settings-data:confirm-import-action"
            >
              Yes, import key
            </AlertDialogAction>
          </AlertDialogFooter>
