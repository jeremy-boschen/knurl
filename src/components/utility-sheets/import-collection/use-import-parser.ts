...
const EMPTY_OPENAPI_OPTIONS: OpenApiToNativeOptions = Object.freeze({})

const useStableOptions = (options: OpenApiToNativeOptions) => {
  const normalized = options && Object.keys(options).length > 0 ? options : EMPTY_OPENAPI_OPTIONS
  const ref = useRef<OpenApiToNativeOptions>(normalized)
  if (!isEqual(ref.current, normalized)) {
...
export function useImportParser(
  ...
  const stableOpenApiOptions = useStableOptions(openApiOptions)
  ...
  useEffect(() => {
    ...
    if (format === "openapi") {
      if (isOpenApiDocument(parsedSource)) {
        nativeDoc = openApiToNative(parsedSource, stableOpenApiOptions)
  ...
  }, [importData, importFormat, stableOpenApiOptions])
