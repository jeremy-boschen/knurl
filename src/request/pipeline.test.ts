  ...
  const makeState = (overrides?: Partial<ApplicationState>): ApplicationState => {
    const base: ApplicationState = {
      collectionsState: {
        ...
        cache: {
          "col-1": {
            ...
            authentication: {
              type: "oauth2",
              oauth2: { grantType: "client_credentials", tokenCaching: "always" } as any,
            },
          },
        },
      },
      ...
      credentialsCacheApi: {
        ...
        get: vi.fn(async function (this: any, _key: string) {
          return undefined
        }),
        remove: vi.fn(),
        clear: vi.fn(),
    ...
    base.collectionsApi = {
      getCollection: (collectionId: string) => {
        const collection = base.collectionsState.cache[collectionId]
        if (!collection) {
          throw new Error(`Collection ${collectionId} not found in test state`)
        }
        return collection
      },
    } as any
