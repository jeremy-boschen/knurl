...
const CookieRow = ({
  ...
  return (
    <DataTableRow data-test-id={`cookie-list:row:${cookie.name}`}>
      ...
      <DataTableCell type="cell" className="flex items-center gap-2">
        <Button
          variant="ghost"
          size="icon"
          onClick={() => onCopy(cookieValue)}
          title="Copy cookie"
          data-test-id={`cookie-list:copy-button:${cookie.name}`}
        >
        ...
        <Button
          variant="ghost"
          size="icon"
          onClick={() => onAddToCookies(cookie)}
          title="Add to request cookies"
          data-test-id={`cookie-list:add-to-request-button:${cookie.name}`}
        >
...
export const CookieList = ({ tabId, cookies }: { tabId: string; cookies: Cookie[] }) => {
  ...
  return (
    <DataTable
      columnTemplate="minmax(120px,1fr) minmax(240px,3fr) minmax(140px,1fr) minmax(80px,0.7fr) minmax(200px,1fr) minmax(100px,1fr) minmax(100px,0.8fr)"
      data-test-id="cookie-list"
    >
