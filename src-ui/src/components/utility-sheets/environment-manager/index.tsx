...
export default function EnvironmentManager({ collectionId, selectedEnvironmentId }: EnvironmentManagerProps) {
  ...
  const handleEnvironmentAction = (environment: Environment, action: EnvironmentAction) => {
    switch (action) {
      case "select":
      case "rename":
        setState({
      ...
      case "duplicate": {
        ...
        if (environment.description) {
          api.updateEnvironment(collection.id, newEnv.id, {
        ...
        for (const variable of sourceVariables) {
          api.addEnvironmentVariable(collection.id, newEnv.id, {
            name: variable.name,
          ...
          <Alert className="mb-3">
            ...
            <AlertDescription>
              <p>
                ...
                <code className="font-mono text-primary">{varNameSyntax}</code> in URLs, params, headers, and bodies.
                Only enabled variables substitute; unresolved placeholders remain.
              </p>
            </AlertDescription>
          </Alert>
        </div>
