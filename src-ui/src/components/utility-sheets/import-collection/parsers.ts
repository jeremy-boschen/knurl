...
const POSTMAN_SUPPORTED_SCHEMA_FRAGMENT = "/json/collection/v2.1"
const POSTMAN_SUPPORTED_BODY_MODES = ["raw", "urlencoded", "formdata", "binary", "file", "graphql"] as const
const POSTMAN_SUPPORTED_AUTH_TYPES = ["basic", "bearer", "apikey", "apiKey", "oauth2", "noauth", "inherit"] as const
const supportedPostmanBodyModes = new Set<string>(POSTMAN_SUPPORTED_BODY_MODES)
const supportedPostmanAuthTypes = new Set<string>(POSTMAN_SUPPORTED_AUTH_TYPES.map((type) => type.toLowerCase()))
const POSTMAN_SUPPORTED_AUTH_MESSAGE = ["bearer", "basic", "apiKey", "oauth2", "inherit", "noauth"] as const
...
export const validatePostmanDocument = (doc: unknown): PostmanValidationResult => {
  ...
  if (result.success) {
    const issues = ensureSupportedPostmanCollection(result.data)
    if (issues.length > 0) {
...
function ensureSupportedPostmanCollection(collection: PostmanCollection): z.ZodIssue[] {
  const issues: z.ZodIssue[] = []
  const schema = collection.info.schema ?? ""
  if (!schema.includes(POSTMAN_SUPPORTED_SCHEMA_FRAGMENT)) {
    issues.push({
      code: "custom",
      message: "Only Postman Collection v2.1 exports are supported. Re-export your collection using the v2.1 format.",
      path: ["info", "schema"],
    })
  }

  const walkItems = (items: PostmanItem[] | undefined, path: (string | number)[]) => {
    ...
    items.forEach((item, index) => {
      ...
      if (item.request) {
        ...
        if (typeof bodyMode === "string" && bodyMode.trim()) {
          const normalizedMode = bodyMode.toLowerCase()
          if (!supportedPostmanBodyModes.has(normalizedMode)) {
            issues.push({
              code: "custom",
              message: `Request body mode "${bodyMode}" is not supported. Supported modes: ${POSTMAN_SUPPORTED_BODY_MODES.join(", ")}.`,
              path: [...itemPath, "request", "body", "mode"],
            })
          }
        }
        ...
        if (typeof authType === "string" && authType.trim()) {
          ...
          if (!supportedPostmanAuthTypes.has(normalizedAuth)) {
            issues.push({
              code: "custom",
              message: `Postman auth type "${authType}" is not supported. Supported types: ${POSTMAN_SUPPORTED_AUTH_MESSAGE.join(", ")}.`,
              path: [...itemPath, "request", "auth", "type"],
            })
          }
        }
      }
      ...
      if (Array.isArray(item.item)) {
        walkItems(item.item, [...itemPath, "item"])
      }
    })
  }
...
const convertBody = (body: PostmanBody | undefined): Partial<RequestBodyData> | undefined => {
  ...
  const declaredMode = typeof body.mode === "string" ? body.mode.toLowerCase() : undefined
  const mode = declaredMode ?? (body.raw ? "raw" : undefined)
  if (!mode) {
    return { type: "none" }
  }
  if (mode === "raw") {
...
/**
 * Converts a Knurl collection to OpenAPI 3.1.0 specification
 * Reverse mapping of openApiToNative conversion
 */
export function nativeToOpenApi(collection: Collection): OpenAPIObject {
  // 1) Extract servers from environments with baseUrl variable
  const servers = (Object.values(collection.environments ?? {}) ?? []).map((env) => {
    const baseUrlVar = Object.values(env.variables ?? {}).find((v) => v.name === "baseUrl")
    if (!baseUrlVar) {
      return undefined
    }

    const serverVars: Record<string, { default: string; description?: string }> = {}
    for (const [, variable] of Object.entries(env.variables ?? {})) {
      if (variable.name !== "baseUrl") {
        serverVars[variable.name] = {
    ...
    return {
      ...
      ...(Object.keys(serverVars).length > 0 ? { variables: serverVars } : {}),
    }
  })

  // 2) Build paths from requests
  const paths: Record<string, PathItemObject> = {}
  const folderToTag = new Map<string, string>()

  // Map folder IDs to their tag names
  for (const [folderId, folder] of Object.entries(collection.folders ?? {})) {
    if (folderId !== RootCollectionFolderId) {
      folderToTag.set(folderId, folder.name)
    }
  }

  // Build paths from requests
  for (const [, request] of Object.entries(collection.requests ?? {})) {
    // Denormalize {{baseUrl}} path back to plain path
    const pathStr = (request.url ?? "").replace(/^\{\{baseUrl\}\}/, "").replace(/\{\{([^}]+)\}\}/g, "{$1}")

    const method = (request.method?.toLowerCase() ?? "get") as keyof PathItemObject
    ...
    // Add path parameters
    for (const [, param] of Object.entries(request.pathParams ?? {})) {
      parameters.push({
        name: param.name,
        in: "path",
        required: true,
    ...
    for (const [, param] of Object.entries(request.queryParams ?? {})) {
      parameters.push({
        name: param.name,
        in: "query",
    ...
    if (request.body?.type !== "none" && request.body?.content) {
      const contentType =
        request.body.language === "graphql"
          ? "application/graphql"
          : request.body.language === "json"
            ? "application/json"
            : request.body.language === "xml"
              ? "application/xml"
              : request.body.language === "html"
                ? "text/html"
                : request.body.language === "yaml"
                  ? "application/yaml"
                  : "text/plain"

      try {
        const example = request.body.language === "json" ? JSON.parse(request.body.content) : request.body.content
        requestBody = {
          required: true,
          content: {
            [contentType]: {
        ...
        // If parsing fails, just use the raw string as example
        requestBody = {
          required: true,
          content: {
            [contentType]: {
    ...
    // Get tag from folder
    const tag = request.folderId ? folderToTag.get(request.folderId) : undefined
  ...
  // 3) Filter out undefined servers
  const validServers = servers.filter((s): s is ReturnType<(typeof servers)[number]> => s !== undefined)

  // 4) Assemble the OpenAPI spec
  return {
    openapi: "3.1.0",
    info: {
    ...
    ...(validServers.length > 0 ? { servers: validServers } : {}),
    paths,
  }
}
