...
export function useImportActions(
  ...
  const handleImport = useCallback(
    (name: string) => {
      ...
      try {
        setStatus(null)
        const newCollection = collectionsApi().importCollection(filtered, name)
  ...
  const handleOverwrite = useCallback(
    (name: string) => {
      ...
      try {
        setStatus(null)
        collectionsApi().removeCollection(existing.id)
        const newCollection = collectionsApi().importCollection(filtered, name)
        const importedRequests = Object.keys(newCollection.requests ?? {}).length
        const importedEnvironments = Object.keys(newCollection.environments ?? {}).length
        setStatus({
  ...
  const handleMerge = useCallback(
    (name: string) => {
      ...
      try {
        setStatus(null)
        const result = collectionsApi().mergeCollection(existing.id, filtered)
        setStatus({
          kind: "success",
