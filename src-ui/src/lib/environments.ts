...
function getVariableMap(environment: Environment | undefined): Record<string, string> {
  ...
  return Object.values(environment.variables).reduce(
    (acc, variable: EnvironmentVariable & { enabled?: boolean }) => {
      if (!variable.name) {
        return acc
      }
...
export function resolveRequestVariables<T extends object>(request: T, environment: Environment | undefined): T {
  ...
  if ("options" in resolvedRequest && resolvedRequest.options) {
    const options = resolvedRequest.options as {
      caPath?: string
      hostOverride?: string
      ipOverride?: string
    ...
    if (typeof options.caPath === "string") {
      options.caPath = substituteVariables(options.caPath, variables)
    }
    if (typeof options.hostOverride === "string") {
      options.hostOverride = substituteVariables(options.hostOverride, variables)
    }
    if (typeof options.ipOverride === "string") {
      options.ipOverride = substituteVariables(options.ipOverride, variables)
    }
    if (typeof options.timeoutSecs === "string") {
      options.timeoutSecs = substituteVariables(options.timeoutSecs, variables)
    }
    if (typeof options.userAgent === "string") {
      options.userAgent = substituteVariables(options.userAgent, variables)
    }
  }
