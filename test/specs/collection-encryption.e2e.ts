...
describe("Collection Encryption & At-Rest Storage", () => {
  ...
  it("persists collection with per-collection encryption keys", async () => {
    ...
    const created = await callBridge("create_collection", {
      name: collectionName,
    } as any)
  ...
  it("stores collection data on filesystem", async () => {
    ...
    const appDir = await callBridge("getAppDataDir" as any)
    ...
    const rawData = await callBridge("loadAppData", fileName)
  ...
  it("maintains encryption key isolation between collections", async () => {
    ...
    const col1 = await callBridge("create_collection", {
      name: col1Name,
    })
    const col2 = await callBridge("create_collection", {
      name: col2Name,
    })
    ...
    const updated1 = await callBridge("update_collection", {
      id: col1.id,
      authentication: auth1,
    })
    const updated2 = await callBridge("update_collection", {
      id: col2.id,
    ...
    const reloaded1 = await callBridge("get_collection", { id: col1.id })
    const reloaded2 = await callBridge("get_collection", { id: col2.id })
  ...
  it("tolerates partial/corrupted collection files gracefully", async () => {
    ...
    await callBridge("update_collection", {
      id: created.id,
      authentication: auth,
    })
    ...
    const retrieved = await callBridge("get_collection", { id: created.id })
  ...
  it("loads collections from disk on app restart simulation", async () => {
    ...
    const reloaded = await callBridge("get_collection", { id: created.id })
